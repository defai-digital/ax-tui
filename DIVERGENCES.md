...
| `stdin-parser-timeout`     | Terminal input           | The pending-escape-sequence assembly timeout is 100ms (configurable via `stdinParserTimeoutMs`), not 20ms, so a terminal reply split across two reads under load isn't abandoned mid-sequence and re-typed as literal text. | `check:tui-patches` and patch tests       |
