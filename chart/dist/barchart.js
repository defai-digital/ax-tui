 ...
 * width are dropped.
 */
export function layoutBarChart(options, width, height) {
    const barWidth = options.barWidth ?? 1;
    const barGap = options.barGap ?? 1;
    if (width <= 0 || height <= 0 || barWidth <= 0 || barGap < 0 || options.data.length === 0)
        return [];
    ...
    if (options.max !== undefined && Number.isFinite(options.max)) {
        ...
        for (const bar of bars)
            computed = Math.max(computed, bar.value);
        max = computed;
    }
    ...
    for (const bar of bars) {
        ...
        for (let row = barsHeight - 1; row >= 0; row--) {
            ...
            if (char !== BAR_EMPTY) {
                for (let column = 0; column < barWidth; column++)
                    cells.push({ x: x + column, y: row, char, fg: color });
            }
