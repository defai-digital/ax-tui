...
import { getPoint, strokeSegment } from "./painter.js";
import { BOX_BOTTOM_LEFT, BOX_BOTTOM_RIGHT, BOX_TOP_LEFT, BOX_TOP_RIGHT, LINE_BOTTOM_LEFT, LINE_HORIZONTAL, LINE_VERTICAL, } from "./symbols.js";
 ...
 */
export function layoutChart(options, width, height) {
    return layoutChartInternal(options, width, height).cells;
}
/** Internal variant exposing the computed graph area (used by tests). */
export function layoutChartInternal(options, width, height) {
    validateChartOptions(options);
    const cells = [];
    if (width <= 0 || height <= 0)
        return { cells, graphLeft: 0, graphWidth: 0, graphHeight: 0 };
    ...
    if (options.backgroundColor !== undefined) {
        for (let y = 0; y < height; y++) {
            for (let x = 0; x < width; x++)
                cells.push({ x, y, char: " ", bg: options.backgroundColor });
        }
    }
    // --- Bounds resolution -------------------------------------------------
    ...
    if (yLabels.length > 0) {
        ...
        for (const label of yLabels)
            slot = Math.max(slot, label.length);
        if (xLabels.length > 0) {
            ...
            if (alignment === "left")
                slot += Math.max(firstWidth - 1, 0);
            else if (alignment === "center")
                slot += Math.floor(firstWidth / 2);
        }
    ...
    if (labelRowX !== null && xLabels.length >= 2) {
        ...
        if (shownFirst.length > 0) {
            let fx = areaX;
            if (alignment === "left")
                fx = areaX + areaWidth - shownFirst.length;
            else if (alignment === "center")
                fx = areaX + Math.floor((areaWidth - shownFirst.length) / 2);
        ...
        for (let i = 1; i < n - 1; i++) {
            ...
            if (lx < graphLeft || lx + shown.length > width)
                continue;
        ...
        if (last.length <= width) {
            ...
            if (lx >= graphLeft)
                paintText(cells, last, lx, labelRowX, labelColor);
        }
    }
    if (axisColY !== null && yLabelWidth > 0 && yLabels.length >= 2 && graphHeight >= 2) {
        ...
        for (let i = 0; i < n; i++) {
            ...
            let tx = 0;
            if (alignment === "right")
                tx = yLabelWidth - text.length;
            else if (alignment === "center")
                tx = Math.floor((yLabelWidth - text.length) / 2);
    ...
    const layers = [];
    for (const dataset of datasets) {
        const datasetPoints = dataset.data.filter(isFinitePair);
        if (datasetPoints.length === 0)
            continue;
        const grid = createGrid(dataset.marker ?? "braille", graphWidth, graphHeight);
        const graphType = dataset.graphType ?? "scatter";
        const color = dataset.color ?? "white";
        const paint = (px, py) => grid.paint(px, py, color);
        ...
        if (graphType === "line") {
            ...
            for (const [wx, wy] of datasetPoints)
                strokeSegment(wx, 0, wx, wy, bounds, grid, paint);
        }
    ...
    if (graphHeight > 2) {
        ...
        if (xTitle !== undefined && xTitle.length > 0 && xTitle.length < graphWidth) {
            ...
            if (row < height)
                paintText(cells, xTitle, width - xTitle.length, row, xAxis?.color ?? axisColor);
        }
    ...
    if (legendPosition !== "none") {
        ...
        if (named.length > 0) {
            ...
            for (const dataset of named)
                innerWidth = Math.max(innerWidth, dataset.name.length);
            ...
            if (innerWidth > 0 && legendWidth <= maxWidth && legendHeight <= maxHeight && heightMargin >= 0) {
                ...
                if (legendPosition === "top-right" && legendWidth + yTitleWidth > graphWidth)
                    ly += 1;
                else if (legendPosition === "top-left" && yTitleWidth > 0)
                    ly += 1;
                else if (legendPosition === "bottom-left" && xTitleWidth + legendWidth > graphWidth)
                    ly -= 1;
                else if (legendPosition === "bottom-right" && xTitleWidth > 0)
                    ly -= 1;
