...
export function nativeMetadata(packageRoot, target) {
    if (!isNativeTarget(target))
        throw new Error(`Unsupported ax-tui native target: ${target}`);
...
function verifyBytes(bytes, expectedHash, maxSize, exactSize, label) {
    ...
    if (digest(bytes) !== expectedHash)
        throw new Error(`ax-tui native artifact SHA-256 mismatch: ${label}`);
}
function readArtifact(file, maxSize) {
    const stat = lstatSync(file);
    if (!stat.isFile() || stat.size > maxSize)
        throw new Error(`Invalid ax-tui native artifact: ${file}`);
    return readFileSync(file);
}
export function verifyNativeArtifacts(directory, entry) {
    if (!lstatSync(directory).isDirectory())
        throw new Error(`Invalid ax-tui native directory: ${directory}`);
    const paths = artifactPaths(directory, entry);
    verifyBytes(readArtifact(paths.libraryPath, entry.lib.size), entry.lib.sha256, entry.lib.size, entry.lib.size, "library");
    verifyBytes(readArtifact(paths.licensePath, MAX_LICENSE_BYTES), entry.licenseSha256, MAX_LICENSE_BYTES, undefined, "LICENSE");
    return paths;
}
function defaultCacheDir() {
    if (process.env.AX_CODE_TUI_NATIVE_CACHE_DIR)
        return path.resolve(process.env.AX_CODE_TUI_NATIVE_CACHE_DIR);
...
async function download(url, maxSize, signal, fetchImpl) {
...
export async function prepareNativeLibraryFrom(packageRoot, target, options = {}, fetchImpl = globalThis.fetch) {
    ...
    if (existsSync(vendored.libraryPath)) {
        ...
        for (const file of Object.values(vendored)) {
            if (!lstatSync(file).isFile())
                throw new Error(`Invalid bundled ax-tui native artifact: ${file}`);
        }
        return vendored;
    }
