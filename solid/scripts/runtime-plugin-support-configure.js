...
function assertCompatibleInstall(install, modules, options) {
  for (const specifier of Object.keys(modules)) {
    if (!install.specifiers.has(specifier)) {
      throw new Error(
        `AX Code TUI Solid runtime plugin support is already installed without ${specifier}. Call ensureRuntimePluginSupport({ additional }) from ax-tui/solid/runtime-plugin-support/configure before importing ax-tui/solid/runtime-plugin-support.`,
      )
    }
  }
