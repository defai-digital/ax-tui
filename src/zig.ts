...
import { prepareNativeLibrary, type NativeTarget } from "../native/index.js"
...
// Native assets are owned and released by ax-tui. No upstream package lookup.
function resolveVendoredNativeTarget(): NativeTarget {
  if (
...
const vendoredNativeTarget = resolveVendoredNativeTarget()
...
let targetLibPath = (await prepareNativeLibrary(vendoredNativeTarget)).libraryPath
...
// Floor cell origins before strict u32 FFI marshalling.
function ffiCellOrigin(x: number, y: number): { x: number; y: number } | null {
  x = Math.floor(x)
  y = Math.floor(y)
...
class FFIRenderLib implements RenderLib {
  private native: ReturnType<typeof openRenderLibrary>
  ...
  public bufferGetCharPtr(buffer: Pointer): Pointer {
    const ptr = this.native.symbols.bufferGetCharPtr(buffer)
    if (!ptr) {
  ...
  public bufferGetFgPtr(buffer: Pointer): Pointer {
    const ptr = this.native.symbols.bufferGetFgPtr(buffer)
    if (!ptr) {
  ...
  public bufferGetBgPtr(buffer: Pointer): Pointer {
    const ptr = this.native.symbols.bufferGetBgPtr(buffer)
    if (!ptr) {
  ...
  public bufferGetAttributesPtr(buffer: Pointer): Pointer {
    const ptr = this.native.symbols.bufferGetAttributesPtr(buffer)
    if (!ptr) {
  ...
  public bufferSetCellWithAlphaBlending(
    ...
    this.native.symbols.bufferSetCellWithAlphaBlending(buffer, x, y, charPtr, fg, bg, attributes ?? 0)
  }
  ...
  public bufferSetCell(
    ...
    this.native.symbols.bufferSetCell(buffer, x, y, charPtr, fg, bg, attributes ?? 0)
  }
  ...
  public bufferFillRect(buffer: Pointer, x: number, y: number, width: number, height: number, color: RGBA) {
    ...
    this.native.symbols.bufferFillRect(buffer, x, y, width, height, bg)
  }
  ...
  public createTextBufferView(textBuffer: TextBufferHandle): TextBufferViewHandle {
    const viewPtr = this.native.symbols.createTextBufferView(textBuffer) as TextBufferViewHandle
    if (!viewPtr) {
  ...
  public textBufferViewSetSelection(
    ...
    this.native.symbols.textBufferViewSetSelection(view, start, end, bg, fg)
  }
  ...
  public textBufferViewSetLocalSelection(
    ...
    const bg = optionalRgbaPtr(bgColor)
    const fg = optionalRgbaPtr(fgColor)
    return Boolean(this.native.symbols.textBufferViewSetLocalSelection(view, anchorX, anchorY, focusX, focusY, bg, fg))
  }

  public textBufferViewUpdateSelection(view: Pointer, end: number, bgColor: RGBA | null, fgColor: RGBA | null): void {
    const bg = optionalRgbaPtr(bgColor)
    const fg = optionalRgbaPtr(fgColor)
    this.native.symbols.textBufferViewUpdateSelection(view, end, bg, fg)
  }
  ...
  public textBufferViewUpdateLocalSelection(
    ...
    const fg = optionalRgbaPtr(fgColor)
    return Boolean(
      this.native.symbols.textBufferViewUpdateLocalSelection(view, anchorX, anchorY, focusX, focusY, bg, fg),
    )
  }
  ...
  public editorViewGetTextBufferView(view: EditorViewHandle): TextBufferViewHandle {
    const result = this.native.symbols.editorViewGetTextBufferView(view) as TextBufferViewHandle
    if (!result) {
  ...
  public createEditBuffer(widthMethod: WidthMethod): EditBufferHandle {
    const widthMethodCode = widthMethod === "wcwidth" ? 0 : 1
    const bufferPtr = this.native.symbols.createEditBuffer(widthMethodCode, this.eventSinkPtr ?? 0) as EditBufferHandle
    if (!bufferPtr) {
  ...
  public editBufferGetTextBuffer(buffer: EditBufferHandle): TextBufferHandle {
    const result = this.native.symbols.editBufferGetTextBuffer(buffer) as TextBufferHandle
    if (!result) {
  ...
  public editorViewSetSelection(
    ...
    const bg = optionalRgbaPtr(bgColor)
    const fg = optionalRgbaPtr(fgColor)
    this.native.symbols.editorViewSetSelection(view, start, end, bg, fg)
  }
  ...
  public editorViewUpdateSelection(view: Pointer, end: number, bgColor: RGBA | null, fgColor: RGBA | null): void {
    const bg = optionalRgbaPtr(bgColor)
    const fg = optionalRgbaPtr(fgColor)
    this.native.symbols.editorViewUpdateSelection(view, end, bg, fg)
  }
  ...
  public bufferDrawChar(
    ...
    this.native.symbols.bufferDrawChar(buffer, char, x, y, rgbaPtr(fg), rgbaPtr(bg), attributes)
  }
  ...
  public createSyntaxStyle(): SyntaxStyleHandle {
    const styleHandle = this.native.symbols.createSyntaxStyle() as SyntaxStyleHandle
    if (!styleHandle) {
...
let renderLibPath: string | undefined
let renderLib: RenderLib | undefined
